...
class GenerationIntegrationModule:
    ...
    def contextualize_query(self, query: str, chat_history: List[Tuple[str, str]]) -> str:
        """
        根据对话历史将用户问题改写为独立查询
        """
        ...
        # 将历史格式化为字符串
        history_str = "\n".join([f"用户: {h[0]}\n助手: {h[1]}" for h in chat_history])

        prompt = PromptTemplate(
            template="""
你是一个对话分析助手。给定一段对话历史和一个新的用户问题，这个新问题可能指代了历史中的内容。
请将新问题改写为一个**独立的、完整的搜索查询**，使其不需要上下文也能被理解。
如果问题已经很独立，或与历史无关，请保持原样。
不要回答问题，只输出改写后的查询。
...
改写后的独立查询:""",
        ...
        rewritten_query = chain.invoke(query).strip()
        logger.info(f"多轮对话改写: '{query}' -> '{rewritten_query}'")
        return rewritten_query

    def _build_history_text(self, history: List[Tuple[str, str]], max_turns: int = 5) -> str:
        """构建历史对话文本"""
        if not history:
            return "无历史对话。"
...
分类结果:""")
    ...
    def generate_basic_answer_stream(self, query: str, context_docs: List[Document], history: List[Tuple[str, str]] = None):
        """
        生成基础回答 - 流式输出
        ...
        prompt = ChatPromptTemplate.from_template("""
你是一位专业的烹饪助手。请根据食谱信息和对话历史回答用户的问题。
...
回答:""")
        ...
        chain = (
            {"question": RunnablePassthrough(), "context": lambda _: context, "history_text": lambda _: history_text}
            | prompt
            | self.llm
            | StrOutputParser()
        )
    ...
    def generate_step_by_step_answer_stream(self, query: str, context_docs: List[Document], history: List[Tuple[str, str]] = None):
        """
        生成详细步骤回答 - 流式输出
        ...
        prompt = ChatPromptTemplate.from_template("""
你是一位专业的烹饪导师。请根据食谱信息和对话历史，为用户提供指导。
...
请灵活组织回答。如果用户是在追问某个步骤的细节，请直接针对性回答；如果用户是在询问新菜谱的做法，请按标准结构（介绍、食材、步骤、技巧）回答。 建议包含以下部分（可根据实际内容调整）：
